...
if (isset($_POST['emp_login'])) {
  $emp_login = $_POST['emp_email']; // This will store either email or username
  $emp_pwd = sha1(md5($_POST['emp_pwd'])); //double encrypt to increase security
  $stmt = $mysqli->prepare("SELECT emp_email ,emp_pwd , emp_id FROM obrs_employee WHERE (emp_email=? OR emp_uname=?) and emp_pwd=? "); //sql to log in user
  $stmt->bind_param('sss', $emp_login, $emp_login, $emp_pwd); //bind fetched parameters
  $stmt->execute(); //execute bind
  $stmt->bind_result($emp_email, $emp_pwd, $emp_id); //bind result
  $rs = $stmt->fetch();
  $_SESSION['emp_id'] = $emp_id; //assaign session to employee id
  
  if ($rs) { //if its sucessfull
    if(isset($_POST['remember_me'])) {
      // Set cookies that expire in 30 days
      setcookie('emp_email', $_POST['emp_email'], time() + (86400 * 1), "/");
      setcookie('emp_pwd', $_POST['emp_pwd'], time() + (86400 * 1), "/"); 
    }
...
$saved_email = isset($_COOKIE['emp_email']) ? $_COOKIE['emp_email'] : '';
$saved_pwd = isset($_COOKIE['emp_pwd']) ? $_COOKIE['emp_pwd'] : '';
?>
<!DOCTYPE html>
<html lang="en">
    ...
    <div class="w-full max-w-md">
      <div class="bg-white/90 backdrop-blur-lg rounded-2xl shadow-2xl overflow-hidden">
        <div class="p-8 text-center">
          ...
          <h2 class="mt-4 text-lg font-semibold text-blue-800">Employee Login Portal</h2>
        </div>
