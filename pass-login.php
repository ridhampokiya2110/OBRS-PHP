...
if (isset($_POST['pass_login'])) {
  $pass_login = $_POST['pass_email']; // This will store either email or username
  $pass_pwd = sha1(md5($_POST['pass_pwd'])); //double encrypt to increase security
  $stmt = $mysqli->prepare("SELECT pass_email, pass_pwd, pass_id FROM obrs_passenger WHERE (pass_email=? OR pass_uname=?) and pass_pwd=? "); //sql to log in user with email or username
  $stmt->bind_param('sss', $pass_login, $pass_login, $pass_pwd); //bind fetched parameters
  ...
  if ($rs) { //if its sucessfull
    if(isset($_POST['remember_me'])) {
      ...
      setcookie('pass_email', $_POST['pass_email'], time() + (86400 * 1), "/");
      setcookie('pass_pwd', $_POST['pass_pwd'], time() + (86400 * 1), "/"); 
    }
    $success = "Login Successful! Redirecting...";
    header("refresh:2;url=pass-dashboard.php");
  } else {
...
$saved_email = isset($_COOKIE['pass_email']) ? $_COOKIE['pass_email'] : '';
$saved_pwd = isset($_COOKIE['pass_pwd']) ? $_COOKIE['pass_pwd'] : '';
?>
<!DOCTYPE html>
<html lang="en">
    ...
    <div class="w-full max-w-md">
      <div class="bg-white/90 backdrop-blur-lg rounded-2xl shadow-2xl overflow-hidden">
        <div class="p-8 text-center">
          ...
          <h2 class="mt-4 text-lg font-semibold text-blue-800">Welcome back! Please login to continue</h2>
        </div>
        ...
        <div class="p-8 pt-0">
          <form method="POST">
            <div class="space-y-6">
              ...
              <p class="mt-2 text-sm text-center text-gray-600">Don't have an account? <a href="pass-signup.php" class="text-emerald-500 hover:text-emerald-600 font-medium">Sign up</a></p>
            </div>
          </form>
        </div>
      </div>
